...
{
  init_platform();
  usart_init(MGT_USART, 9600);
  // pwm_initTimer() and pwm_enableChannel() must be call before using the magnetorquer functions on the specific coil's channel

  while (1)
  {
    usart_transmitChar(MGT_USART, 'K');
    nop(1000);
  //   char command[] = {'0'};
  //   uint8_t coil_num[] = {-1};
  //   uint8_t timer_num[] = {-1};
